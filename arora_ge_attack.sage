...
def LWE_polynomial (q, d):
    ...
    :param q    -- Ordine del campo finito in cui vivono i coefficienti del polinomio
...
def oracle_LWE(u, alpha, bool=False, d=1):
    ...
    :param bool (= False)   -- Se bool=True prendo eta con una distribuzione uniforme su [-d;+d] 
    :param d (= 1)          -- Se bool=True prendo eta con una distribuzione uniforme su [-d;+d] 
    :return (a,b)           -- Scelgo a uniformemente in GF(q)^N, scelgo b = a*u + eta dove eta segue la 
                               distribuzione scelta (uniforme o gaussiana discreta)
    """
    n = len(u)
...
def arora_ge_attack(q, n, alpha, C=1, bool=False):
    ...
    print('Ho ottenuto un sistema polinomiale in %d variabili' % (len(exp_list) - 1))
